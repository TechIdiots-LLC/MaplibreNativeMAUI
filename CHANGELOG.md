...
- **Windows: drag-to-rotate/pitch compass** — The navigation compass button on the Windows overlay can now be dragged: horizontal drag rotates the map bearing (via `atan2` about the button centre) and vertical drag adjusts pitch (clamped 0–60°); a click (≤4 px of movement) still resets bearing to north.  The compass needle icon rotates to reflect the current bearing, and the GPS bearing-reset icon uses a ↺ glyph.
